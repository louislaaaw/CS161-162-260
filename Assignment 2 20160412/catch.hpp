...
class CommandLine {
    ...
    public:
        ...
        ArgBinder& argName( std::string const& argN ) {
            m_arg.argName = argN;
            return *this;
        }
...
public:
    ...
    void optUsage( std::ostream& os, std::size_t indent = 0, std::size_t width = CATCH_CONFIG_CONSOLE_WIDTH ) const {
        ...
        for( it = itBegin; it != itEnd; ++it ) {
            Catch::Text use( it->commands(), Catch::TextAttributes()
            ...
            for( std::size_t i = 0; i < (std::max)( use.size(), desc.size() ); ++i ) {
                std::string usageCol = i < use.size() ? use[i] : "";
                os << usageCol;
...
public:
    ...
    explicit RunContext( Ptr<IConfig const> const& conf, Ptr<IStreamingReporter> const& reporter )
        :   m_runInfo( conf->name() ),
